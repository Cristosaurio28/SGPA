...
class UsuarioController extends AbstractController {
    public function registro(): string {
        ...
        if (!($params->has('nombre') && $params->has('clave') && $params->has('tipo')  && $params->has('email'))) {
        ...
        try {
                $usuarioModel->addUsuario(new Usuario(0, $nombre, $tipo, $clave, $email));
        } catch (DbException $e) {
    ...
    public function login(): string {
        ...
        if (!($params->has('email') && $params->has('clave'))) {
