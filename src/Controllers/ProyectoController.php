...
class ProyectoController extends AbstractController {
    public function registro(): string {
        ...
        if (!($params->has('titulo') && $params->has('estado')&& $params->has('archivo')&& 
        $params->has('url'))) {
            $params = ['errorMessage' => 'No info provided.'];
            return $this->render('proyecto.twig', $params);
        }

        $titulo = $params->getString('titulo');
        $estado = $params->getString('estado');
        $archivo = $params->getString('archivo');
        $url = $params->getString('url');

        $proyectoModel = new ProyectoModel($this->db);

        try {
            $proyectoModel->addDocente(new Proyecto(0, $titulo, $estado, $archivo, $url));
        } catch (DbException $e) {
             $properties = ['errorMessage' => $e->getMessage()];
              return $this->render('proyecto.twig', $properties);
        }
    ...
    public function login(): string {
        ...
        if (!($params->has('titulo') && $params->has('estado')&& $params->has('archivo')&& $params->has('url'))) {
        ...
        $docenteModel = new ProyectoModel($this->db);

        try {
            $docente= $docenteModel->getByTitulo($titulo);
            if ($docente->getestado() != $estado)
        ...
        $_SESSION['usuario_nombre'] = $docente->getTitulo();
